...
- [ ] 4.5 Manual, Safari (including installed-PWA context if practical): confirm either working mic capture or a clean fallback to typed-only — no broken/error UI state — **blocked on a real access problem, not a code question:** Robin's iPhone Safari can only reach production `vocare.ca`, which doesn't have this branch's code yet since nothing had been pushed to `main`. Decision (`chat`/Robin, per `cli`'s explicit hand-off): push and deploy now rather than set up LAN/tunnel exposure to local dev — the code is already extensively manually verified across three other browsers, the mic control fails safe by design (3.4) if Safari's behavior differs, and a secure-context requirement (Web Speech API needs HTTPS, `localhost`-only exception) would have undermined a same-LAN HTTP alternative anyway. Still genuinely untested pending Robin's actual iPhone Safari pass against the now-live site.
