...
- Whether the mining extraction prompt's static portion clears Haiku 4.5's 4096-token cacheable-prefix minimum, and whether caching is worth adding given the once-per-session (not once-per-turn) call pattern — measured, not assumed, per Decisions above. **Measured 2026-07-22 (tasks.md 7.6):** the static system prompt (no target_role addendum) is 438 tokens via the real `messages.countTokens` endpoint — well below the 4096-token minimum, same outcome M2's own prompt hit. No `cache_control` added, per the Decisions section's default.
