...
- **Local Postgres (dev):** Docker, `postgres:16-alpine`, mapped to host port `5434` (per `docker-compose.yml`, provisioned in M1). **Corrected 2026-07-22:** originally assigned `5433` for this exact reason — avoiding native-Postgres collisions — but that missed a native **PostgreSQL 17** Windows service (in addition to 18) already listening on `5433` itself; discovered when `drizzle-kit migrate` silently authenticated against the wrong server. `5434` confirmed free and now in use.
...
- **Domain — `vocare.ca` live since 2026-07-21.** Registrar and DNS host: **Web Hosting Canada (WHC.ca)** — DNS managed in WHC's own Zone Editor, not delegated to Vercel/Cloudflare nameservers. Apex is canonical (Vercel's default apex→`www` redirect was declined); `www` CNAMEs to the apex. `api.vocare.ca` (Railway) is not set up yet — needed by M1. **Resend — decided 2026-07-21, corrects an earlier "sending subdomain" plan:** verifying the apex `vocare.ca` itself, not a subdomain (`backend/src/auth/send-magic-link.ts` already defaults `from` to `hello@vocare.ca`; SPF/DKIM TXT/CNAME records coexist fine with the existing apex A record). Not set up yet.
