...
- **Domain — `vocare.ca` live since 2026-07-21.** Registrar and DNS host: **Web Hosting Canada (WHC.ca)** — DNS managed in WHC's own Zone Editor, not delegated to Vercel/Cloudflare nameservers. Apex is canonical (Vercel's default apex→`www` redirect was declined); `www` CNAMEs to the apex. `api.vocare.ca` (Railway) and a Resend sending subdomain are not set up yet — needed by M1, not before.
