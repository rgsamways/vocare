...
- **Fair-use enforcement — mechanism decided 2026-07-21.** A soft, *undisclosed* velocity cap (rolling-window rate limit — e.g., a max sessions/24h and a max sessions/30-day window), not a published lifetime session count. A visible lifetime cap would directly contradict the "$29, unlimited, forever" pitch that differentiates Vocare from subscription competitors (Section 8's competitive scan); an unpublished "reasonable use" threshold — the same pattern unlimited mobile data plans and AWS's fair-use clauses use — stays technically unlimited while still bounding the AAirpass-style outlier risk from Section 5/11. Enforced at the same server-side entitlement check immediately above, not a separate system. **Placeholder thresholds only, not final:** sized generously enough that a real interview-prep cadence (2-3 sessions/day for a couple weeks) never trips it, restrictive enough to catch genuine outlier usage or Section 17's free-tier-jailbreak-as-general-chatbot pattern — sanity-check the actual numbers against real beta usage before launch rather than guessing with zero data. **The exact ToS wording is not finalized here** — flag for the same lawyer already recommended for the PIPEDA/content-licensing clauses (Section 3, Section 20), not drafted as binding legal text by this document. **Decided 2026-07-22 — user-facing behavior on trip, previously undecided:** a plain in-app message ("you've hit today's practice limit, come back tomorrow" or similar, worded around the undisclosed threshold without revealing the exact number), not silent throttling. Someone who paid $29 and gets blocked with zero explanation is a support/trust problem waiting to happen — the cap needs to stay undisclosed as a *number*, not invisible as an *event*.
...
- **Gap found in an earlier drift audit — Section 13's account-deletion finding was never actually built anywhere.** It lived as a data-model prose note (Section 3) but no module had an actual task for it. Adding here: implement account deletion as a real cascade-delete across `users`, `sessions`, `transcript_turns`, `session_mining_results`, `feedback_reports`, `tier1_profiles`, **and `anchors`/`anchor_revisions` (added 2026-07-22 — the deletion list predated the anchors feature and was never updated when it landed)** — and per Section 3's own note, decide the Tier 2b recomputation-vs-disclosure approach as part of this task, not separately.
