...
- **Additional pricing tiers alongside the $29 one-time — floated 2026-07-22, not decided.** The idea: a $14/month subscription and a $2 pay-per-session option, alongside the existing 3-free-sessions-then-$29-once model. Not evaluated for viability yet — flagged here mainly because it's a real M1 rework (M1's entitlement logic is a strict binary, no subscription state, no charge-at-session-start path) that would also touch whatever session-start entitlement check M2 ends up calling into, so it needs a proper spec pass of its own rather than being layered in ad hoc. Revisit as a deliberate follow-on change once M2 has landed.
- **Referral program — floated 2026-07-22, not decided.** The idea: a referrer (e.g. a specific friend) earns a percentage (discussed: 10%) of every paid signup attributed to them via email/referral code. Not evaluated for viability yet — like the pricing-tiers idea above, this is a real M1 billing change, not a toggle: needs (1) an attribution mechanism capturing the referral code/email at signup and persisting it through to eventual payment, which can happen weeks later after the free sessions run out; (2) a commission calculation off the existing `checkout.session.completed` webhook; (3) a payout decision — automating actual money movement to a referrer means Stripe Connect (onboarding them as a connected account, split payments, tax-reporting obligations), a heavy lift versus just tracking attributed conversions in a report and paying out manually. Also couples to the pricing-tiers idea above, since referral math would need to work across whatever tiers exist. Revisit as a deliberate follow-on change once M2 has landed, same as pricing tiers.
